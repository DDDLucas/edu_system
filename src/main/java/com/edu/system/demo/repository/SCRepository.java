...
public interface SCRepository extends JpaRepository<SelectedCourse, Integer> {
    ...
    @Query(value="SELECT c.id,c.name,c.teacher_id,c.stu_num,c.type,c.speciality,c.institute,c.class_time,c.classroom,c.credit,c.max_num,sc.id as sc_id FROM selected_course sc,course c WHERE sc.course_id=c.id AND sc.stu_id =stu_id",nativeQuery = true)
